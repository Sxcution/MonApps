...
# =============================================================================
# CRITICAL FIX: Đảm bảo working directory luôn là thư mục chứa script
# Fix cho vấn đề double-click từ Windows Explorer
# =============================================================================
try:
    # Lấy đường dẫn tuyệt đối của script hiện tại
    if getattr(sys, 'frozen', False):
        # PyInstaller bundle - dùng executable path
        script_dir = os.path.dirname(sys.executable)
    else:
        # Script thông thường - dùng __file__ path
        script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Đổi working directory về thư mục chứa script
    if os.path.exists(script_dir):
        os.chdir(script_dir)
        # Log working directory change (nếu cần debug)
        # logging.basicConfig(level=logging.DEBUG)
        # logging.debug(f"Changed working directory to: {script_dir}")
except Exception as e:
    # Nếu có lỗi khi đổi working directory, vẫn tiếp tục chạy
    # Nhưng log lỗi ra file để debug
    try:
        error_log = os.path.join(os.path.dirname(__file__) if '__file__' in globals() else os.getcwd(), "working_dir_error.log")
        with open(error_log, "w", encoding="utf-8") as f:
            f.write(f"Error changing working directory: {e}\n")
...
# CRITICAL: Khi chạy với pythonw.exe (không có console), sys.stdout/stderr là None
# Chỉ wrap nếu stdout/stderr không None (có console)
if sys.platform == "win32":
    # Chỉ wrap stdout nếu không None (có console - python.exe)
    if sys.stdout is not None and hasattr(sys.stdout, 'buffer'):
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
    # Chỉ wrap stderr nếu không None (có console - python.exe)
...
try:
    ...
    if os.path.exists(notes_module_path):
        ...
        HAS_NOTES = False
        print("Warning: Không tìm thấy notes_module.py")
except Exception as e:
    HAS_NOTES = False
    print(f"Warning: Không load được notes_module: {e}")
...
class MainHub(QMainWindow):
    ...
    def add_notes_tab(self):
        ...
        try:
            ...
            icon = get_icon('note')
            if icon:
                self.tabs.addTab(notes_widget, icon, "Ghi Chú")
            else:
                self.tabs.addTab(notes_widget, "📝 Ghi Chú")
            logging.info("✅ Notes module loaded successfully")
        except Exception as e:
            error_widget = self.create_error_widget(
                "📝 Ghi Chú",
                f"Lỗi khi load Notes module:\n{str(e)}"
            )
            icon = get_icon('note')
            if icon:
                self.tabs.addTab(error_widget, icon, "Ghi Chú")
            else:
                self.tabs.addTab(error_widget, "📝 Ghi Chú")
